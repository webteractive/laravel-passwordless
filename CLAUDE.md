...
- **One table.**
  ...
  - **Foreign keys differ between the two tables on purpose.** `passwordless_challenges.user_id` has **no** constraint — the user model is configurable, so the table cannot assume the referenced table is named `users` or that its key is a bigint. Prune handles orphans. `passwordless_social_accounts` **does** use `->constrained()`, so the social strategy (and only it) requires a bigint `users` table. Do not "fix" the asymmetry by adding `->constrained()` to challenges; `MigrationsTest` pins it.
  - `user_id` is a `foreignId` (unsigned bigint), so a user model with a **non-integer key writes a uuid into a bigint column**. SQLite tolerates it, MySQL will not. `UuidUser` is only exercised through dev-login today, which writes no challenge row — so this is untested rather than known-good.
